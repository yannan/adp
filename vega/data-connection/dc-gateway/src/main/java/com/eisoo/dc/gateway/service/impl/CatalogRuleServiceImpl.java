...
import com.eisoo.dc.gateway.ruleConfig.RuleConfigCache;
import com.eisoo.dc.gateway.ruleConfig.OperatorConfig;
import com.eisoo.dc.gateway.domain.dto.CatalogRuleDto;
import com.eisoo.dc.gateway.domain.dto.CatalogTypeDto;
import com.eisoo.dc.gateway.domain.dto.RuleDto;
import com.eisoo.dc.gateway.domain.vo.HttpResInfo;
import com.eisoo.dc.gateway.domain.vo.RuleNameVo;
...
public class CatalogRuleServiceImpl implements CatalogRuleService {
    ...
    public ResponseEntity<?> OperatorList(String operator) {
        OperatorConfig operatorConfig =ruleConfigCache.getRuleConfig(operator);
    ...
    public ResponseEntity<?> configRule(CatalogRuleDto catalogRuleDto, String user) {
        ...
        if(StringUtils.isNotEmpty(catalogRuleDto.getCatalogName())){
            String result=showCatalogInfo(serviceEndpoints.getVegaCalculateCoordinator(),catalogRuleDto.getCatalogName());
        ...
        String catalogListStr = showCatalogList(serviceEndpoints.getVegaCalculateCoordinator());
        int count=0;
        ...
        if(StringUtils.isEmpty(catalogRuleDto.getCatalogName()) && StringUtils.isNotEmpty(catalogRuleDto.getDatasourceType())){
            ...
            for (String name : str){
                ...
                postCatalogRule(serviceEndpoints.getVegaCalculateCoordinator(), catalogRuleDto, user);
                count++;
            }
        }else{
            postCatalogRule(serviceEndpoints.getVegaCalculateCoordinator(), catalogRuleDto, user);
    ...
    public ResponseEntity<?> RuleList() {
        Set<String> connectorName=ruleConfigCache.getRuleNames();
