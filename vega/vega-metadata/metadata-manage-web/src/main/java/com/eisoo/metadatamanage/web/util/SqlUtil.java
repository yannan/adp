...
public class SqlUtil {
    public static List<String> getTable(String sql, DbType dbType) {
        ...
        for (int i = 0; i < stmtList.size(); i++) {
            ...
            for (Map.Entry<TableStat.Name, TableStat> row : visitor.getTables().entrySet()) {
                tableNames.add(row.getKey().getName());
            }
        }
        return tableNames;
    }

    public static List<String> getColumn(String sql, DbType dbType) {
        if (sql.contains("*")) {
        ...
        for (int i = 0; i < stmtList.size(); i++) {
            SQLStatement stmt = stmtList.get(i);
            AishuSchemaStatVisitor visitor = new AishuSchemaStatVisitor(dbType);
            stmt.accept(visitor);

            for (TableStat.Column row : visitor.selectColumns) {
                columns.add(row.getFullName());
            }
        }
        return columns;
    }

    static class AishuSchemaStatVisitor extends SQLASTVisitorAdapter {
        ...
        protected final List<TableStat.Column> selectColumns = new ArrayList<>();
        ...
        public AishuSchemaStatVisitor() {
            this((DbType) null);
        }

        public AishuSchemaStatVisitor(SchemaRepository repository) {
            if (repository != null) {
        ...
        public AishuSchemaStatVisitor(DbType dbType) {
            this(new SchemaRepository(dbType), new ArrayList<Object>());
            this.dbType = dbType;
        }

        public AishuSchemaStatVisitor(List<Object> parameters) {
            this((DbType) null, parameters);
        }

        public AishuSchemaStatVisitor(DbType dbType, List<Object> parameters) {
            this(new SchemaRepository(dbType), parameters);
            this.parameters = parameters;
        }

        public AishuSchemaStatVisitor(SchemaRepository repository, List<Object> parameters) {
            this.repository = repository;
            this.parameters = parameters;
            if (repository != null) {
        ...
        protected TableStat.Column addColumn(String tableName, String columnName, SQLObjectImpl x) {
            TableStat.Column c = new TableStat.Column(tableName, columnName, dbType);
            if (x != null && isSelectItem(x)) {
                selectColumns.add(new TableStat.Column(tableName, columnName));
            }
        ...
        protected TableStat.Column addColumn(SQLName table, String columnName, SQLObjectImpl x) {
            String tableName;
            ...
            if (isSelectItem(x)) {
                selectColumns.add(new TableStat.Column(tableName, columnName, columnHashCode64));
            }
        ...
        private void orderByAddColumn(String table, String columnName, SQLObject expr) {
            TableStat.Column column = new TableStat.Column(table, columnName, dbType);
        ...
        protected TableStat.Column getColumn(SQLExpr expr) {
            ...
            if (expr instanceof SQLIdentifierExpr) {
                ...
                if (table != null) {
                    ...
                    final TableStat.Column old = columns.get(columnHashCode64);
                    if (old != null) {
                        return old;
                    }
            ...
            if (tableSource instanceof SQLExprTableSource) {
                ...
                if (expr instanceof SQLIdentifierExpr) {
                    ...
                    if (schemaObject != null
                        ...
                        if (columnDef == null) {
                            column = addColumn("UNKNOWN", ident, x);
                        }
                    }

                    if (column == null) {
                        column = addColumn(table, ident, x);
                    }
                    ...
                    if ("table".equalsIgnoreCase(methodInvokeExpr.getMethodName())
                        ...
                        column = addColumn(tableName, ident, x);
                    }
                }
            } else if (tableSource instanceof SQLWithSubqueryClause.Entry
        ...
        private boolean isSelectItem(SQLObject parent) {
        ...
        protected TableStat.Column handleUnknownColumn(String columnName) {
            return addColumn("UNKNOWN", columnName, null);
        }
            ...
            if (createStmt != null
                ...
                for (SQLTableElement e : createStmt.getTableElementList()) {
                    if (e instanceof SQLColumnDefinition) {
                        ...
                        TableStat.Column column = addColumn(tableName, columnName.toString(), x);
                ...
                TableStat.Column column = addColumn(expr, "*", x);
                if (isParentSelectItem(x.getParent())) {
                    column.setSelec(true);
                }
            }
        ...
        public boolean visit(SQLUpdateSetItem x) {
            ...
            if (column != null) {
                ...
                final TableStat.Column columnStat = getColumn(column);
                if (columnStat != null) {
        ...
        public boolean visit(SQLColumnDefinition x) {
            ...
            TableStat.Column column = addColumn(tableName, columnName, x);
        ...
        public boolean visit(SQLAlterTableAddColumn x) {
            ...
            for (SQLColumnDefinition column : x.getColumns()) {
                ...
                addColumn(stmt.getName(), columnName, x);
            }
            return false;
        }
        ...
        public boolean visit(SQLDescribeStatement x) {
            ...
            if (column != null) {
                ...
                this.addColumn(tableName, columnName, x);
            }
            return false;
        }
        ...
        public boolean visit(SQLCopyFromStatement x) {
            ...
            if (table != null) {
                ...
                for (SQLName column : x.getColumns()) {
                    addColumn(table.getName(), column.getSimpleName(), x);
                }
            }
