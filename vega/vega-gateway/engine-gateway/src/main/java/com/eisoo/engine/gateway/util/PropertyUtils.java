...
public class PropertyUtils {
    ...
    static {
        loadPropertyFile(CatalogConstant.HIVE_PROPERTIES_PATH);
    }
    ...
    public static String getUpperCaseString(String key) {
        ...
        return StringUtils.isBlank(val) ? val : val.toUpperCase();
    }
    ...
    public static Map<String, String> getPrefixedHiveProperties(String prefix) {
        Map<String, String> matchedProperties = new HashMap<>();
        for (String propName : properties.stringPropertyNames()) {
            if (propName.startsWith(prefix)) {
                String endPropName = StringUtils.substring(propName, propName.indexOf(".") + 1, propName.length());
                matchedProperties.put(endPropName, properties.getProperty(propName));
            }
        }
        return matchedProperties;
    }
