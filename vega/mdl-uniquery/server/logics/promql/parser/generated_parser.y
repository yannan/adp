...
%type <node> step_invariant_expr aggregate_expr aggregate_modifier bin_modifier left_join_modifier out_join_modifier binary_expr bool_modifier expr function_call function_call_args function_call_body group_modifiers label_matchers matrix_selector number_literal offset_expr on_or_ignoring left_join out_join paren_expr string_literal subquery_expr unary_expr vector_selector
...
binary_expr     : expr ADD     bin_modifier expr { $$ = yylex.(*parser).newBinaryExpression($1, $2, $3, $4) }
                ...
                | expr ADD     left_join_modifier expr { $$ = yylex.(*parser).newBinaryExpression($1, $2, $3, $4) }
                | expr SUB     left_join_modifier expr { $$ = yylex.(*parser).newBinaryExpression($1, $2, $3, $4) }
                | expr ADD     out_join_modifier expr { $$ = yylex.(*parser).newBinaryExpression($1, $2, $3, $4) }
                | expr SUB     out_join_modifier expr { $$ = yylex.(*parser).newBinaryExpression($1, $2, $3, $4) }
                ;
...
left_join  : bool_modifier LEFT_JOIN grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.MatchingLabels = $3
                        $$.(*BinaryExpr).VectorMatching.LeftJoin = true
                        }
                ;

left_join_modifier    : left_join GROUP_LEFT maybe_grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.Card = LeftJoinManyToMany
                        $$.(*BinaryExpr).VectorMatching.Include = $3
                        $$.(*BinaryExpr).VectorMatching.ExistGroupLeft = true
                        }
                | left_join
                        {
                        $$ = $1
...
out_join  : bool_modifier OUT_JOIN grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.MatchingLabels = $3
                        $$.(*BinaryExpr).VectorMatching.OutJoin = true
                        }
                ;

out_join_modifier    : out_join SELECT_LEFT maybe_grouping_labels SELECT_RIGHT maybe_grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.Card = OutJoinManyToMany
                        $$.(*BinaryExpr).VectorMatching.IncludeLeft = $3
                        $$.(*BinaryExpr).VectorMatching.ExistGroupLeft = true
                        $$.(*BinaryExpr).VectorMatching.IncludeRight = $5
                        $$.(*BinaryExpr).VectorMatching.ExistGroupRight = true
                        }
                | out_join SELECT_LEFT maybe_grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.Card = OutJoinManyToMany
                        $$.(*BinaryExpr).VectorMatching.IncludeLeft = $3
                ...
                | out_join SELECT_RIGHT maybe_grouping_labels
                        {
                        $$ = $1
                        $$.(*BinaryExpr).VectorMatching.Card = OutJoinManyToMany
                        $$.(*BinaryExpr).VectorMatching.IncludeRight = $3
                        $$.(*BinaryExpr).VectorMatching.ExistGroupRight = true
                        }
                | out_join
                        {
...
matrix_selector : expr LEFT_BRACKET AUTO RIGHT_BRACKET
                        {
                        var errMsg string
...
maybe_label     : AVG | BOOL | BOTTOMK | BY | COUNT | COUNT_VALUES | GROUP | GROUP_LEFT | GROUP_RIGHT | SELECT_LEFT | SELECT_RIGHT | LEFT_JOIN | OUT_JOIN | IDENTIFIER | IGNORING | LAND | LOR | LUNLESS | MAX | METRIC_IDENTIFIER | MIN | OFFSET | ON | QUANTILE | STDDEV | STDVAR | SUM | TOPK | START | END;
