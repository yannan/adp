...
CREATE TABLE IF NOT EXISTS t_metric_model (
  ...
  f_measure_name varchar(40) NOT NULL DEFAULT '' COMMENT '度量名称',
  ...
  f_analysis_dimessions varchar(1024) DEFAULT NULL COMMENT '分析维度',
...
CREATE TABLE IF NOT EXISTS t_data_view (
  ...
  f_create_time bigint(20) NOT NULL DEFAULT 0 COMMENT '创建时间',
  f_update_time bigint(20) NOT NULL DEFAULT 0 COMMENT '更新时间',
  f_data_source text NOT NULL COMMENT '数据视图数据源',
  f_field_scope tinyint(2) NOT NULL DEFAULT '0' COMMENT '字段范围: 0 部分字段, 1 全部字段',
  f_fields longtext NOT NULL COMMENT '字段和读时解析',
  f_filters text NOT NULL COMMENT '过滤条件',
  f_builtin tinyint(2) DEFAULT 0 COMMENT '内置视图标识: 0 非内置, 1 内置',
  f_open_streaming tinyint(2) NOT NULL DEFAULT 0 COMMENT '是否开启视图实时订阅任务: 0 不开启, 1 开启',
  f_job_id varchar(40) NOT NULL DEFAULT '' COMMENT '订阅任务 id',
  f_loggroup_filters longtext DEFAULT NULL COMMENT '日志分组过滤条件',
  f_group_id varchar(40) NOT NULL DEFAULT '' COMMENT '数据视图分组 id',
  PRIMARY KEY (f_view_id),
  UNIQUE KEY uk_f_view_name (f_group_id, f_view_name)
...
CREATE TABLE IF NOT EXISTS t_data_dict_item (
  ...
  f_dict_id varchar(40) NOT NULL COMMENT '数据字典id',
  f_item_key varchar(255) NOT NULL COMMENT '数据字典项key值',
  f_item_value varchar(255) NOT NULL COMMENT '数据字典项value值',
