...
docLibTypeMap = {'user_doc_lib': 1, 'department_doc_lib': 2, 'custom_doc_lib': 3, 'knowledge_doc_lib': 6}
...
def fetch_process_config(proc_def_id: tuple) -> dict:
    url = "http://workflow-rest-private:9801/api/workflow-rest/v1/process-model-internal/{}".format(proc_def_id[0])
    ...
    if response.status  == 200:
        ...
        # 请求失败
        raise RequestException(f'[fetch_process_config] Request failed with status code: {response.status_code, proc_def_id[0]}')

def get_share_auditors(proc_def_id: tuple) -> list:
    entries = []
    url = "http://workflow-rest-private:9801/api/workflow-rest/v1/doc-share-strategy-internal/?doc_type=&proc_def_id={}".format(proc_def_id[0])
    response = request.urlopen(url)
    if response.status  == 200:
        # 请求成功
        data = response.read()
        ...
        # 请求失败
        raise RequestException(f'[get_share_auditors] Request failed with status code: {response.status_code, proc_def_id[0]}')
...
def create_stratege(process_config, auditor, advance_config) -> str:
    ...
    req = request.Request(url=url, data=request_body_json, headers=headers, method='POST')
    response = request.urlopen(req)
    if response.status  == 200:
        return key
    else:
        # 请求失败
        raise RequestException(f'[create_stratege] Request failed with status code: {response.status_code, str(request_body_json)}')
...
def migration_data() -> dict:
    ...
    for stratege in stratege_datas:
        try:
            # 流程信息
            process_config =retry_function(fetch_process_config, 3, 1, stratege)
            # 流程对应的审核员信息
        ...
        for auditor in auditors:
            try:
                key = retry_function(create_stratege, 3, 1, process_config, auditor, dict_datas)
                share_stratege_info = [auditor.get("doc_id"), auditor.get("doc_name"), auditor.get("doc_type"), key]
                if process_config.get("key") == 'Process_SHARE001':
                    new_share_stratege_map['real_name'].append(share_stratege_info)
                else:
                    new_share_stratege_map['anonymous'].append(share_stratege_info)
            except Exception as e:
                print(e)
                continue
...
def update_audit_policy(conn_cursor):
    ...
    for userTypeStr in typeArry:
        ...
        for info in user_share_stratege_info:
            ...
            """
            select_policy_sql = select_policy_sql.format(userType, info[0], doc_lib_type)
            ...
            insert_policy_sql = """
                ...
                """
            sqlStr  = insert_policy_sql.format(userType,doc_lib_id , doc_lib_name , doc_lib_type, info[3][:16], timestamp)
            conn_cursor.execute(sqlStr)
    return

def get_user_doc_lib_id(conn_cursor, user_id):
    doc_lib_id = ""
    GET_USER__DOC_SQL = """select f_doc_id from anyshare.t_acs_doc where f_creater_id = '{}';""".format(user_id)
...
def retry_function(func, max_retries=3, delay=1, *args, **kwargs):
    for _ in range(max_retries):
        try:
            ...
            print(f"[retry_function] Function {func.__name__} Exception caught: {e}")
            time.sleep(delay)
    raise Exception(f"[retry_function] Function {func.__name__} exceeded maximum retry attempts")
