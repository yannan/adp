...
def column_exists(conn_cursor, table_name, column_name):

    # 查询字段是否存在
    query = "SHOW COLUMNS FROM `{}` LIKE '{}'".format(table_name, column_name)
...
def add_col(conn_cursor, table_name, column_name, sql):
    if column_exists(conn_cursor, table_name, column_name):
        pass
    else:
        add_query = sql
...
if __name__ == "__main__":
    ...
    try:
        add_col(conn_cursor, "t_wf_doc_share_strategy", "transfer_switch",
                "ALTER TABLE `t_wf_doc_share_strategy` ADD COLUMN `transfer_switch` varchar(10) DEFAULT NULL NULL COMMENT '转审开关';")
        add_col(conn_cursor, "t_wf_doc_share_strategy", "transfer_count",
                "ALTER TABLE `t_wf_doc_share_strategy` ADD COLUMN `transfer_count` varchar(10) DEFAULT NULL NULL COMMENT '最大转审次数';")
